...
const ClienteDetalhe: React.FC = () => {
  ...
  const TabButton: React.FC<{tabName: string; label: string}> = ({tabName, label}) => (
      <button 
        ...
        className={`px-4 py-2 text-sm font-medium rounded-t-lg transition-colors border-b-2 ${activeTab === tabName ? 'border-primary text-primary' : 'border-transparent text-subtle hover:border-gray-300'}`}
      >
          {label}
      </button>
  );
             ...
             <Card>
                <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-6 text-sm">
                    <div className="space-y-4">
                        <div className="flex items-center"><Mail className="w-4 h-4 mr-3 text-primary" /><span>{cliente.email}</span></div>
                        <div className="flex items-center"><Phone className="w-4 h-4 mr-3 text-primary" /><span>{cliente.phone}</span></div>
                        <div className="flex items-center"><Building className="w-4 h-4 mr-3 text-primary" /><span>{cliente.cnpj || 'Não informado'}</span></div>
                         <div className="flex items-center"><MapPin className="w-4 h-4 mr-3 text-primary" /><span>{cliente.endereco || 'Não informado'}</span></div>
                    </div>
                     <div className="space-y-4">
                        <div className="flex items-center"><UserIcon className="w-4 h-4 mr-3 text-primary" /><span>{cliente.responsavel || 'Não informado'}</span></div>
                ...
                <CardContent>
                  {contratos.length > 0 ? (
                    <ul className="space-y-3">
                      {contratos.map(c => (
                        <li key={c.id} className="flex justify-between items-center text-sm p-3 rounded-md bg-background border border-border">
